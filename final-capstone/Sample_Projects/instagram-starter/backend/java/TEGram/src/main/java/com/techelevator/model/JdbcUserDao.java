...
public class JdbcUserDao implements UserDao {
    ...
    private final static String[] colors1 = new String[] { "FAEBD7", "F0FFFF", "FFE4C4", "7FFF00", "00CED1", "FFD700", "FAFAD2", "66CDAA", "AFEEEE", "C0C0C0" };
    private final static String[] colors2 = new String[] { "262626", "D2691E", "7FFF00", "556B2F", "483D8B", "2F4F4F", "8B4513", "663399", "800000" };
    ...
    public User saveUser(String username, String password, String role, String image) {
        ...
        createDefaultImage(newUser);
        byte[] salt = passwordHasher.generateRandomSalt();
        String hashedPassword = passwordHasher.computeHash(password, salt);
        String saltString = new String(Base64.encode(salt));
        long newId = jdbcTemplate.queryForObject("INSERT INTO users(username, password, salt, role, image) VALUES (?, ?, ?, ?, ?) RETURNING id", 
        		Long.class, newUser.getUsername(), hashedPassword, saltString, newUser.getRole(), newUser.getImage());
        newUser.setId(newId);
        return newUser;
    }
	...
	static private void createDefaultImage(User user) {
        if (user.getImage() == null || user.getImage().length() == 0) {
            ...
            String color = colors1[rand.nextInt(colors1.length)]; // - 1)];
            String bgcolor = colors2[rand.nextInt(colors2.length)]; // - 1)];
            user.setImage("https://ui-avatars.com/api?name=" + 
                    user.getUsername().trim().replace(' ', '+') +
                    "&length=2&size=128&rounded=true&color=" + 
                    color +
                    "&background=" +
                    bgcolor +
